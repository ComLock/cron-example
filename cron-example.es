...
const jobs = [{
  cron: '0 0 * * 1',
  fn: () => {
    log.info('Once every monday at 00:00');
  }
}, {
  cron: '* * * * *',
  fn: () => {
...
jobs.push({
  cron: '0 * * * *',
  fn: () => {
    log.info('Once every hour');
  }
});

cron.start(); // At this point the two original jobs will get called.
cron.update(jobs); // At this point the original jobs are gone and only 'Once every hour' gets called.
